...
class FMPFilterTester:
    ...
    async def test_filters(self, filters: Dict[str, str], description: str) -> Dict[str, Any]:
        ...
        try:
            ...
            print()
            return {"status": "success", "count": count, "data": data[:3] if count > 0 else []}
...
async def run_filter_tests():
    ...
    print("=" * 80)
    print("ANALYSIS:")
    print("• If volume threshold significantly affects count → Volume is the limiting factor")
    print("• If market cap only gives much higher count → Price/volume filters are restrictive")
