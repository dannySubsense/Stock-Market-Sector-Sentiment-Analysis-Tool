...
async def test_production_universe_builder():
    ...
    print(f"   Market Cap: ${universe_builder.market_cap_min:,} - ${universe_builder.market_cap_max:,}")
    print(f"   Min Volume: {universe_builder.min_daily_volume:,} shares")
    print(f"   Exchanges: {universe_builder.valid_exchanges}")
    print(f"   Price Limits: {universe_builder.min_price} - {universe_builder.max_price}")
    print()
    ...
    if result['status'] == 'success':
        ...
        if stocks:
            ...
            print(f"   ✅ Small-cap stocks (${10_000_000:,} - ${2_000_000_000:,}): {small_cap_count}")
