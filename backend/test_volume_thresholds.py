    ...
    try:
        for test_name, volume_threshold in volume_tests:
            ...
            if volume_threshold is None:
                ...
                reduction_pct = ((baseline_size - universe_size) / baseline_size) * 100 if 'baseline_size' in locals() else 0
            
            results.append((test_name, volume_threshold, universe_size, reduction_pct))
            
            print(f"📊 Result: {universe_size} stocks")
            if volume_threshold:
                print(f"📉 Reduction: -{baseline_size - universe_size} stocks ({reduction_pct:.1f}%)")
        ...
        for test_name, volume_threshold, universe_size, reduction_pct in results:
            ...
            print(f"   {test_name:20} | {volume_str:>10} | {universe_size:>5} stocks | {reduction_pct:>5.1f}% reduction")
        
        print(f"\n💡 RECOMMENDATIONS:")
        print(f"   📊 For Maximum Coverage: No volume filter ({results[0][2]} stocks)")
        print(f"   ⚖️  For Balanced Approach: 100K-250K threshold (~{[r[2] for r in results if r[1] in [100000, 250000]]}")
        print(f"   🎯 For High Liquidity: 500K+ threshold")
        print(f"   ❌ Current 1M threshold too restrictive: only {results[-1][2]} stocks")
