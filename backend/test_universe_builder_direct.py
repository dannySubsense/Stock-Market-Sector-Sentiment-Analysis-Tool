...
async def test_universe_builder_direct():
    ...
    print(f"   Market Cap Range: ${universe_builder.market_cap_min:,} - ${universe_builder.market_cap_max:,}")
    ...
    try:
        ...
        if universe_result["status"] == "success":
            ...
            print(f"   📊 vs Target (1500): {'🎯 Close!' if 1200 <= universe_size <= 1800 else '📈 Different scale'}")
            print(f"   🔗 vs Previous Tests (~2123): {'✅ Consistent' if 2000 <= universe_size <= 2200 else '❓ Different'}")
            ...
            if stocks:
                ...
                for i, stock in enumerate(sample_stocks, 1):
                    ...
                    name = stock.get('companyName', 'N/A')[:30] + "..." if len(stock.get('companyName', '')) > 30 else stock.get('companyName', 'N/A')
                    ...
                    print(f"          📊 Vol: {volume:,}" if isinstance(volume, (int, float)) else f"          📊 Vol: {volume}")
                ...
                prices = [s.get('price', 0) for s in stocks if isinstance(s.get('price'), (int, float))]
                if prices:
                    print(f"   💲 Price Range: ${min(prices):.2f} - ${max(prices):.2f}")
                
                # Volume analysis  
                volumes = [s.get('volume', 0) for s in stocks if isinstance(s.get('volume'), (int, float))]
                if volumes:
                    min_vol, max_vol = min(volumes), max(volumes)
                    print(f"   📊 Volume Range: {min_vol:,} - {max_vol:,}")
                    under_100k = sum(1 for v in volumes if v < 100000)
                    print(f"   ⚠️  Stocks under 100K volume: {under_100k} ({under_100k/len(volumes)*100:.1f}%)")
