    ...
    try:
        ...
        for stock in stocks:
            stock_info = {
                ...
                "last_updated": (
                    stock.last_updated.isoformat() if stock.last_updated else None
                ),
            }
...
async def get_stock_details(symbol: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if not stock:
            raise HTTPException(
                status_code=404, detail=f"Stock '{symbol}' not found in universe"
            )

        # Get current stock data
        stock_data = (
            db.query(StockData).filter(StockData.symbol == symbol.upper()).first()
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get stock details: {str(e)}"
        )
...
async def get_universe_stats(db: Session = Depends(get_db)):
    ...
    try:
        # Get total counts
        total_stocks = (
            db.query(StockUniverse).filter(StockUniverse.is_active == True).count()
        )

        # Get sector breakdown
        sector_stats = (
            db.query(
                StockUniverse.sector, func.count(StockUniverse.symbol).label("count")
            )
            .filter(StockUniverse.is_active == True)
            .group_by(StockUniverse.sector)
            .all()
        )
        ...
        micro_cap_count = (
            db.query(StockUniverse)
            .filter(
                StockUniverse.is_active == True, StockUniverse.market_cap < 300_000_000
            )
            .count()
        )
        ...
        exchange_stats = (
            db.query(
                StockUniverse.exchange, func.count(StockUniverse.symbol).label("count")
            )
            .filter(StockUniverse.is_active == True)
            .group_by(StockUniverse.exchange)
            .all()
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get universe stats: {str(e)}"
        )
...
async def get_gap_stocks(
    ...
    try:
        ...
        query = db.query(StockData).filter(StockData.gap_percent.isnot(None))
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get gap stocks: {str(e)}"
        )
...
async def get_volume_leaders(
    ...
    try:
        ...
        stocks = (
            db.query(StockData)
            .filter(StockData.volume_ratio.isnot(None), StockData.volume_ratio > 1.0)
            .order_by(StockData.volume_ratio.desc())
            .limit(limit)
            .all()
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get volume leaders: {str(e)}"
        )
...
async def refresh_universe(db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to refresh universe: {str(e)}"
        )
