...
async def show_fmp_universe():
    ...
    print("-" * 120)
    print(
        f"{'Symbol':<8} {'Company':<30} {'Market Cap':<15} {'Price':<8} {'Volume':<12} {'Exchange':<8} {'Sector':<15}"
    )
    print("-" * 120)
    ...
    for i, stock in enumerate(stocks[:10]):
        ...
        print(
            f"{symbol:<8} {name:<30} {mc_str:<15} ${price:<7.2f} {vol_str:<12} {exchange:<8} {sector:<15}"
        )
    ...
    sectors = [
        stock.get("sector", "Unknown") for stock in stocks if stock.get("sector")
    ]
    sector_counts = Counter(sectors)

    total_with_sectors = sum(sector_counts.values())
    print(
        f"Stocks with sector data: {total_with_sectors}/{universe_size} ({total_with_sectors/universe_size*100:.1f}%)"
    )
    print()
    ...
    exchanges = [
        stock.get("exchange", "Unknown") for stock in stocks if stock.get("exchange")
    ]
    ...
    market_caps = [
        stock.get("marketCap", 0) for stock in stocks if stock.get("marketCap", 0) > 0
    ]
