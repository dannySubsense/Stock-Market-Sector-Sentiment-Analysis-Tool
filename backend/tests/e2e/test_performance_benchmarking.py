...
class TestE2EPerformanceBenchmarking:
    ...
    @pytest.mark.asyncio
    async def test_e2e_response_time_benchmarking(
        self,
        ...
        for endpoint in test_endpoints:
            ...
            ), f"Validation failed for {endpoint}: {response.status_code}"
        ...
        if all_response_times:
            ...
            validate_e2e_performance(
                average_response_time, target_response_time, "Average Response Time"
            )

            # Validate max response time (should be reasonable)
            max_target = target_response_time * 3  # Allow 3x the target for max
            validate_e2e_performance(
                max_response_time, max_target, "Maximum Response Time"
            )
        ...
        assert (
            total_execution_time < max_benchmark_time
        ), f"Benchmark took {total_execution_time}s, exceeds {max_benchmark_time}s limit"

    @pytest.mark.e2e
    @pytest.mark.performance
    @pytest.mark.asyncio
    async def test_e2e_throughput_benchmarking(
        self,
        async_client: AsyncClient,
        e2e_benchmark_targets: Dict[str, float],
        e2e_test_data: Dict[str, Any],
        ...
        for _ in range(baseline_requests):
            ...
            assert (
                response.status_code == status.HTTP_200_OK
            ), f"Baseline request failed: {response.status_code}"

        baseline_end = time.time()
        baseline_duration = baseline_end - baseline_start
        baseline_throughput = (
            baseline_requests / baseline_duration
        ...
        throughput_results.append(
            ("concurrent", concurrent_throughput, concurrent_duration)
        )
        ...
        for i in range(sustained_requests):
            ...
            assert (
                response.status_code == status.HTTP_200_OK
            ), f"Sustained request {i} failed: {response.status_code}"
        ...
        throughput_results.append(
            ("sustained", sustained_throughput, sustained_duration)
        )
        ...
        assert (
            validation_response.status_code == status.HTTP_200_OK
        ), "Validation failed after throughput test"
    ...
    @pytest.mark.performance
    @pytest.mark.asyncio
    async def test_e2e_load_testing(
        self,
        async_client: AsyncClient,
        e2e_benchmark_targets: Dict[str, float],
        e2e_test_data: Dict[str, Any],
        ...
        baseline_tasks = [
            asyncio.create_task(async_client.get(test_endpoint))
        ...
        baseline_successful = sum(
            1 for result in baseline_results if not isinstance(result, Exception)
        )
        baseline_duration = baseline_end - baseline_start
        load_results.append(
            ("baseline", baseline_users, baseline_successful, baseline_duration)
        )
        ...
        light_successful = sum(
            1 for result in light_results if not isinstance(result, Exception)
        )
        ...
        medium_tasks = [
            asyncio.create_task(async_client.get(test_endpoint))
            for _ in range(medium_users)
        ]
        medium_results = await asyncio.gather(*medium_tasks, return_exceptions=True)
        medium_end = time.time()

        medium_successful = sum(
            1 for result in medium_results if not isinstance(result, Exception)
        )
        medium_duration = medium_end - medium_start
        load_results.append(
            ("medium", medium_users, medium_successful, medium_duration)
        )
        ...
        heavy_successful = sum(
            1 for result in heavy_results if not isinstance(result, Exception)
        )
        ...
        assert (
            success_rate >= 0.9
        ), f"Medium load success rate {success_rate:.2%} below 90% target"

        # Validate system stability (baseline should be 100% successful)
        baseline_load = next(
            result for result in load_results if result[0] == "baseline"
        )
        assert (
            baseline_load[2] == baseline_load[1]
        ), "Baseline load should be 100% successful"
        ...
        performance_stats = {
            "load_testing": {
                ...
                "success_rates": {
                    result[0]: result[2] / result[1] for result in load_results
                },
            }
        }
        ...
        assert (
            total_execution_time < max_benchmark_time
        ), f"Load test took {total_execution_time}s, exceeds {max_benchmark_time}s limit"

    @pytest.mark.e2e
    @pytest.mark.performance
    @pytest.mark.asyncio
    async def test_e2e_stress_testing(
        self,
        async_client: AsyncClient,
        e2e_benchmark_targets: Dict[str, float],
        e2e_test_data: Dict[str, Any],
        ...
        baseline_response_time = await measure_response_time(
            async_client, test_endpoint
        )
        ...
        successful_stress = sum(
            1 for response in stress_responses if not isinstance(response, Exception)
        )
        ...
        ), "System did not recover from stress"
        ...
        post_stress_response_time = await measure_response_time(
            async_client, test_endpoint
        )
        ...
        recovery_result = next(
            result for result in stress_results if result[0] == "recovery"
        )
        validate_e2e_performance(
            recovery_result[1], target_recovery_time, "Stress Recovery Time"
        )

        # Validate stress success rate (should be reasonable)
        stress_result = next(
            result for result in stress_results if result[0] == "stress"
        )
        assert (
            stress_result[1] >= 0.8
        ), f"Stress test success rate {stress_result[1]:.2%} below 80% target"

        # Validate post-stress performance (should be similar to baseline)
        baseline_result = next(
            result for result in stress_results if result[0] == "baseline"
        )
        post_stress_result = next(
            result for result in stress_results if result[0] == "post_stress"
        )

        performance_degradation = (
            post_stress_result[1] - baseline_result[1]
        ) / baseline_result[1]
        assert (
            performance_degradation < 0.5
        ), f"Performance degradation {performance_degradation:.2%} exceeds 50% limit"
        ...
        assert (
            total_execution_time < max_benchmark_time
        ), f"Stress test took {total_execution_time}s, exceeds {max_benchmark_time}s limit"
