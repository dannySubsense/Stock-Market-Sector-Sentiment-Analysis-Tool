...
class TestE2EAPIIntegration:
    ...
    @pytest.mark.asyncio
    async def test_e2e_api_endpoint_chain(self, async_client: AsyncClient,
        ...
        assert response.status_code == status.HTTP_200_OK, f"Health check failed: {response.status_code}"
        ...
        assert response.status_code == status.HTTP_200_OK, f"Sectors endpoint failed: {response.status_code}"
        ...
        assert response.status_code == status.HTTP_200_OK, f"Stocks endpoint failed: {response.status_code}"
        ...
        response_time = await measure_response_time(async_client, "/api/analysis/on-demand", 
                                                   method="POST", json_data={"analysis_type": "full"})
        ...
        assert response.status_code == status.HTTP_200_OK, f"Analysis endpoint failed: {response.status_code}"
        ...
        response = await async_client.get("/api/health")
        assert response.status_code == status.HTTP_200_OK, f"Final health check failed: {response.status_code}"
        ...
        await validate_api_endpoint_chain(expected_chain, executed_endpoints, chain_name)
        
        # Validate performance targets
        target_time = e2e_test_data["performance_targets"]["api_chain_execution"]
        validate_e2e_performance(total_execution_time, target_time, "API Chain Execution")
        ...
        for endpoint, response_time in performance_metrics.items():
            validate_e2e_performance(response_time, api_target, f"{endpoint} Response Time")
    ...
    @pytest.mark.api_integration
    @pytest.mark.asyncio
    async def test_e2e_data_flow_integration(self, async_client: AsyncClient,
        ...
        response = await async_client.get("/api/sectors")
        assert response.status_code == status.HTTP_200_OK, f"Sector data flow failed: {response.status_code}"
        ...
        response = await async_client.get("/api/stocks")
        assert response.status_code == status.HTTP_200_OK, f"Stock data flow failed: {response.status_code}"
        ...
        response = await async_client.post("/api/analysis/on-demand", json={"analysis_type": "full"})
        assert response.status_code == status.HTTP_200_OK, f"Analysis data flow failed: {response.status_code}"
        ...
        validate_e2e_performance(total_execution_time, target_time, "Data Flow Validation")
    ...
    @pytest.mark.api_integration
    @pytest.mark.asyncio
    async def test_e2e_performance_integration(self, async_client: AsyncClient,
        ...
        successful_requests = sum(1 for result in load_results if not isinstance(result, Exception))
        assert successful_requests == len(concurrent_tasks), f"Only {successful_requests}/{len(concurrent_tasks)} concurrent requests succeeded"
        ...
        assert successful_stress == len(stress_responses), f"Only {successful_stress}/{len(stress_responses)} stress requests succeeded"
        ...
        assert recovery_response.status_code == status.HTTP_200_OK, "System not responsive after stress test"
    ...
    @pytest.mark.api_integration
    @pytest.mark.asyncio
    async def test_e2e_error_integration(self, async_client: AsyncClient,
        ...
        try:
            ...
            # Should return 404
            assert response.status_code == status.HTTP_404_NOT_FOUND, f"Invalid endpoint should return 404, got {response.status_code}"
        ...
        try:
            response = await async_client.post("/api/analysis/on-demand", json={"invalid": "data"})
        ...
        recovery_response = await async_client.get("/api/health")
        assert recovery_response.status_code == status.HTTP_200_OK, "System should recover after errors"
        ...
        assert stability_response.status_code == status.HTTP_200_OK, "System should remain stable after errors"
        ...
        assert len(handled_errors) >= len(expected_scenarios) - 1, f"Too few error scenarios handled: {len(handled_errors)}/{len(expected_scenarios)}"
        
        # Validate error recovery performance
        error_recovery_target = e2e_test_data["performance_targets"]["error_recovery"]
        validate_e2e_performance(total_execution_time, error_recovery_target, "Error Recovery")
