...
class TestE2ESystemIntegration:
    ...
    @pytest.mark.asyncio
    async def test_e2e_system_health_integration(
        self,
        ...
        ), f"Basic health check failed: {response.status_code}"
        ...
        if "components" in health_data:
            ...
            for component, component_status in components.items():
                ...
                if (
                    component == "redis"
                    and component_status.get("status") == "unhealthy"
                ):
                    print(
                        f"Warning: Redis component unhealthy - skipping for development environment"
                    )
                    continue
                assert component_status.get("status") in [
                    "healthy",
                    "operational",
                ], f"Component {component} not healthy: {component_status}"
        ...
        if "services" in health_data:
            ...
            for service, service_status in services.items():
                assert service_status.get("status") in [
                    "healthy",
                    "operational",
                ], f"Service {service} not healthy: {service_status}"
        ...
        assert (
            performance_response.status_code == status.HTTP_200_OK
        ...
        assert successful_stability == len(
            stability_checks
        ), f"Stability check failed: {successful_stability}/{len(stability_checks)}"
        ...
        for metric, response_time in performance_metrics.items():
            validate_e2e_performance(
                response_time, target_health_response, f"{metric} Response Time"
            )

        # Validate total execution time
        max_integration_time = e2e_test_data["e2e_timeout"]
        assert (
            total_execution_time < max_integration_time
        ), f"Health integration took {total_execution_time}s, exceeds {max_integration_time}s limit"
    ...
    @pytest.mark.system_integration
    @pytest.mark.asyncio
    async def test_e2e_data_pipeline_integration(
        self,
        async_client: AsyncClient,
        e2e_system_targets: Dict[str, float],
        e2e_test_data: Dict[str, Any],
        ...
        ), f"Data source analysis failed: {analysis_response.status_code}"
        ...
        assert (
            storage_response.status_code == status.HTTP_200_OK
        ), f"Data storage validation failed: {storage_response.status_code}"
        ...
        assert (
            retrieval_response.status_code == status.HTTP_200_OK
        ), f"Data retrieval validation failed: {retrieval_response.status_code}"
        ...
        validate_e2e_performance(
            total_execution_time, target_pipeline_time, "Data Pipeline Execution"
        )
    ...
    @pytest.mark.system_integration
    @pytest.mark.asyncio
    async def test_e2e_cache_integration(
        self,
        async_client: AsyncClient,
        e2e_system_targets: Dict[str, float],
        e2e_test_data: Dict[str, Any],
        ...
        if cache_performance_times:
            average_cache_time = sum(cache_performance_times) / len(
                cache_performance_times
            )
            performance_metrics["cache_performance"] = average_cache_time
            validate_e2e_performance(
                average_cache_time, target_cache_time, "Cache Performance"
            )
        ...
        assert (
            consistency_response1.status_code == status.HTTP_200_OK
        ), "Cache consistency check 1 failed"
        assert (
            consistency_response2.status_code == status.HTTP_200_OK
        ), "Cache consistency check 2 failed"
        ...
        ), "Cache recovery health check failed"
        ...
        assert (
            len(cache_operations) >= 4
        ), f"Too few cache operations completed: {len(cache_operations)}"
    ...
    @pytest.mark.system_integration
    @pytest.mark.asyncio
    async def test_e2e_database_integration(
        self,
        async_client: AsyncClient,
        e2e_system_targets: Dict[str, float],
        e2e_test_data: Dict[str, Any],
        ...
        assert (
            connection_response.status_code == status.HTTP_200_OK
        ...
        assert (
            stocks_response.status_code == status.HTTP_200_OK
        ), "Database data retrieval failed"
        ...
        assert (
            analysis_response.status_code == status.HTTP_200_OK
        ), "Database data storage analysis failed"
        ...
        assert (
            sectors_response.status_code == status.HTTP_200_OK
        ), "Database data consistency check failed"
        ...
        if performance_times:
            ...
            validate_e2e_performance(
                average_db_time, target_db_time, "Database Performance"
            )
        ...
        assert (
            len(db_operations) >= 4
        ), f"Too few database operations completed: {len(db_operations)}"
