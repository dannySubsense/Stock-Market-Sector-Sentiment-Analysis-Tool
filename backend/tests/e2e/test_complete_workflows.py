...
class TestE2EWorkflows:
    ...
    @pytest.mark.asyncio
    async def test_e2e_sector_dashboard_workflow(self, async_client: AsyncClient, 
        ...
        assert response.status_code == status.HTTP_200_OK, f"Failed to load sector grid: {response.status_code}"
        ...
        sector_names = list(sectors_data["sectors"].keys()) if sectors_data["sectors"] else ["technology"]
        test_sector = sector_names[0]
        
        response = await async_client.get(f"/api/sectors/{test_sector}")
        assert response.status_code == status.HTTP_200_OK, f"Failed to get sector details: {response.status_code}"
        ...
        assert response.status_code == status.HTTP_200_OK, f"Failed to get sector stocks: {response.status_code}"
        ...
        response = await async_client.post("/api/analysis/on-demand", json={"analysis_type": "full"})
        assert response.status_code == status.HTTP_200_OK, f"Failed to trigger analysis: {response.status_code}"
        ...
        response = await async_client.get("/api/sectors")
        assert response.status_code == status.HTTP_200_OK, f"Failed to get updated sectors: {response.status_code}"
        ...
        validate_e2e_performance(performance_metrics["sector_grid_load"], target_time, "Sector Grid Loading")
        
        # Validate total workflow time
        max_workflow_time = e2e_test_data["e2e_timeout"]
        assert total_execution_time < max_workflow_time, f"Workflow took {total_execution_time}s, exceeds {max_workflow_time}s limit"
    ...
    @pytest.mark.workflow
    @pytest.mark.asyncio
    async def test_e2e_stock_universe_workflow(self, async_client: AsyncClient,
                                              e2e_workflow_steps: Dict[str, List[str]],
                                              e2e_test_data: Dict[str, Any]):
        """
        ...
        response_time = await measure_response_time(async_client, "/api/stocks/universe/stats")
        performance_metrics["universe_stats_load"] = response_time
        
        response = await async_client.get("/api/stocks/universe/stats")
        assert response.status_code == status.HTTP_200_OK, f"Failed to get universe stats: {response.status_code}"
        
        stats_data = response.json()
        validate_e2e_response_structure(stats_data, "stocks")
        completed_steps.extend(["user_accesses_stock_universe", "system_shows_universe_stats"])
        
        # Step 2: User filters by sector (get stocks with sector filter)
        response_time = await measure_response_time(async_client, "/api/stocks?sector=technology")
        performance_metrics["sector_filter"] = response_time
        
        response = await async_client.get("/api/stocks?sector=technology")
        assert response.status_code == status.HTTP_200_OK, f"Failed to filter stocks by sector: {response.status_code}"
        ...
        if filtered_stocks["stocks"]:
            ...
            assert response.status_code == status.HTTP_200_OK, f"Failed to get stock details: {response.status_code}"
        ...
        assert response.status_code in [status.HTTP_200_OK, status.HTTP_404_NOT_FOUND], f"Unexpected gap stocks response: {response.status_code}"
        ...
        assert response.status_code == status.HTTP_200_OK, f"Failed to refresh universe: {response.status_code}"
        ...
        assert response.status_code == status.HTTP_200_OK, f"Failed to get updated stocks: {response.status_code}"
        ...
        for metric, response_time in performance_metrics.items():
            validate_e2e_performance(response_time, target_time, f"{metric} API Response")
    ...
    @pytest.mark.workflow
    @pytest.mark.asyncio
    async def test_e2e_analysis_workflow(self, async_client: AsyncClient,
                                        e2e_workflow_steps: Dict[str, List[str]],
                                        e2e_test_data: Dict[str, Any]):
        """
        ...
        try:
            ...
            analysis_timeout = e2e_test_data["performance_targets"]["analysis_completion"]
            await asyncio.wait_for(
                self._monitor_analysis_progress(async_client),
        ...
        response = await async_client.get("/api/sectors")
        assert response.status_code == status.HTTP_200_OK, f"Failed to get fresh data: {response.status_code}"
        ...
        target_time = e2e_test_data["performance_targets"]["sector_grid_load"]
        validate_e2e_performance(response_time, target_time, "Post-Analysis Response Time")
        ...
        assert len(completed_steps) >= len(expected_steps) - 1, f"Too few steps completed: {len(completed_steps)}/{len(expected_steps)}"
        
        # Validate analysis performance
        analysis_target = e2e_test_data["performance_targets"]["analysis_completion"]
        assert analysis_duration < analysis_target, f"Analysis took {analysis_duration}s, exceeds {analysis_target}s limit"
    ...
    async def test_e2e_error_recovery_workflow(self, async_client: AsyncClient,
        ...
        response = await async_client.get("/api/sectors")
        assert response.status_code == status.HTTP_200_OK, f"Failed to get cached data: {response.status_code}"
        ...
        assert response.status_code == status.HTTP_200_OK, f"System not responsive: {response.status_code}"
