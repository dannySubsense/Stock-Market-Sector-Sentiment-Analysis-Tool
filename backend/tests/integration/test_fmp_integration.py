...
class TestFMPStockScreenerIntegration:
    ...
    async def test_get_stock_screener_complete_integration(self):
        ...
        if connection_result["status"] != "success":
            pytest.skip(f"FMP API not available: {connection_result.get('message', 'Unknown error')}")
        ...
        if result["status"] == "success":
            ...
            if universe_size > 0:
                ...
                required_fields = ["symbol", "companyName", "marketCap", "price", "exchange"]
                ...
                if market_cap > 0:
                    assert 10_000_000 <= market_cap <= 2_000_000_000, f"Market cap {market_cap} outside range"
                if price > 0:
                    assert 1.00 <= price <= 100.00, f"Price {price} outside range"
                if exchange:
                    assert exchange in ["NASDAQ", "NYSE"], f"Invalid exchange: {exchange}"
            ...
            if universe_size >= 1200:
                ...
                logger.info("⚠️  PARTIAL: Reasonable universe size, may need criteria adjustment")
            else:
                logger.warning(f"⚠️  LOW: Universe size {universe_size} below expectations")
    ...
    async def test_universe_data_quality(self):
        ...
        if result["status"] != "success":
            pytest.skip(f"FMP screener failed: {result.get('message', 'Unknown error')}")
        ...
        assert data_completeness >= 90, f"Low data completeness: {data_completeness:.1f}%"
        ...
        assert expected_exchanges.intersection(found_exchanges), f"Missing target exchanges in {found_exchanges}"
...
class TestFMPPerformanceMetrics:
    ...
    async def test_response_time(self):
        ...
        if result["status"] == "success":
            ...
            assert response_time < 30, f"Response time {response_time:.2f}s exceeds 30s target"
            ...
            logger.info(f"API call failed in {response_time:.2f}s: {result.get('message', 'Unknown error')}")
    ...
    async def test_api_call_efficiency(self):
        ...
        if result["status"] == "success":
            universe_size = result["universe_size"]
            logger.info(f"✅ SUCCESS: Retrieved {universe_size} stocks with single API call")
