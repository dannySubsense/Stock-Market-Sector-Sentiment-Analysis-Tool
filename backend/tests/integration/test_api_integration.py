...
class TestAPIIntegration:
    ...
    async def test_complete_sector_workflow(self, async_client: AsyncClient):
        ...
        if sectors_data["sectors"]:
            ...
            assert "deprecated" in refresh_data.get("status", "") or "message" in refresh_data
    ...
    async def test_analysis_integration(self, async_client: AsyncClient):
        ...
        response1 = await async_client.post("/api/analysis/on-demand", json={"analysis_type": "full"})
    ...
    async def test_performance_under_load(self, async_client: AsyncClient):
        ...
        assert total_time < 10.0, f"5 concurrent requests took {total_time}s, should be < 10s"
    ...
    async def test_data_consistency_across_endpoints(self, async_client: AsyncClient):
        ...
        if "total_stocks" in stats_data and "stocks" in stocks_data:
            ...
            assert stats_total >= actual_count or actual_count <= stocks_data.get("limit", 100)
    ...
    async def test_slice1a_requirements_validation(self, async_client: AsyncClient):
        ...
        if sectors_data["sectors"]:
            ...
            assert actual_sectors == expected_sectors, f"Expected {expected_sectors}, got {actual_sectors}"
        ...
        if sectors_data["sectors"]:
            for sector_name, sector_info in sectors_data["sectors"].items():
                ...
                valid_colors = {"dark_red", "light_red", "blue_neutral", "light_green", "dark_green"}
                assert color in valid_colors, f"Invalid color {color} for sector {sector_name}"
        ...
        assert 0 <= total_stocks <= 2000, f"Universe size {total_stocks} outside expected range"
        ...
        if "sector_breakdown" in stats_data:
            ...
            assert total_in_breakdown == total_stocks, f"Sector breakdown sum {total_in_breakdown} != total {total_stocks}" 